    ...
    private final class ContentReaderThread extends Thread 
    {
        ...
        public void run() 
        {
            ...
            try 
            {
                ...
                if(listener instanceof TwitterQBAuth)
                {
                    String tempoCookies = httpConnection.getHeaderField("Set-Cookie");
                    if ( (tempoCookies != null) && (! tempoCookies.equals("")) )
                    {
                        int pos = tempoCookies.indexOf("_users_session=");    
                        if (pos != -1)
                        {
                            listener.ProcessAnswer(SAVE_BROWSER_SESSION, tempoCookies);
                        }
                    }
                    
                    String tempoTwitterAuthURL = httpConnection.getHeaderField("Location");
                    if ( (tempoTwitterAuthURL != null) && (! tempoTwitterAuthURL.equals("")) )
                    {
                        int pos = tempoTwitterAuthURL.indexOf("https://api.twitter.com/oauth/authenticate?oauth_token=");    
                        if (pos != -1)
                        {
                            listener.ProcessAnswer(SAVE_TWITTER_AUTH_URL, tempoTwitterAuthURL);
                        }
                    }
                }
