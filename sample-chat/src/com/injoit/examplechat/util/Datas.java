...
import com.injoit.examplechat.threads.IWriterThread;
import com.injoit.examplechat.jmc.containers.RoomInfo;
...
public class Datas {
   ...
   // other fields
    ...
    public static Vector romInfos = new Vector();
    ...
    */
    ...
    public static boolean saveRecord() {
       ...
       public static void UpdateInfo(RoomInfo row)
        {
            boolean found = false;
            for(int i=0; i< romInfos.size(); i++)
            {
                RoomInfo info= (RoomInfo) romInfos.elementAt(i);
                if(info.getName().equals(row.getName()) == true)
                {
                    info.setMembersCount(row.getMembersCount());
                    found = true;
                    break;
                }
            }
            ...
            if(found == false)
            {
                romInfos.addElement(row);
            }
        }
        
        public static int GetRoomInfo(String roomName)
        {
            int res = 0;
            boolean found = false;
            for(int i=0; i< romInfos.size(); i++)
            {
                RoomInfo info= (RoomInfo) romInfos.elementAt(i);
                if(info.getName().equals(roomName) == true)
                {
                    res = info.getMembersCount();
                    found = true;
                    break;
                }
            }
